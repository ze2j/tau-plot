...
- A [`CATEGORICAL`](#type) axis displays discrete string labels at fixed positions, one per unique X value in the bound series. 
- A [`CONTINUOUS`](#type) axis maps numeric values to screen positions along a computed domain. 
...
- [`LINEAR`](#scale) distributes values evenly. 
- [`LOGARITHMIC`](#scale) compresses large spans by applying a base-10 logarithm to data values before mapping.
...
| Value | Meaning |
|---|---|
| `LINEAR` | Values are distributed evenly across the axis. |
...
| `FRACTION` | The space is a fraction of the data span, given by [`domain_padding_min`](#domain_padding_min) and [`domain_padding_max`](#domain_padding_max). A value of `0.05` adds 5% of the span. On a [`LOGARITHMIC`](#scale) scale the fraction applies to the log-span instead. |
...
The axis type. Default is [`CONTINUOUS`](#type).

Determines whether the axis renders discrete category labels or a continuous numeric scale. Most other properties only take effect when this is [`CONTINUOUS`](#type).
...
The mapping scale. Default is [`LINEAR`](#scale).

Only used when [`type`](#type) is [`CONTINUOUS`](#type). Controls whether data values are distributed evenly or logarithmically along the axis.
...
Only used when [`type`](#type) is [`CONTINUOUS`](#type) and [`range_override_enabled`](#range_override_enabled) is `false`. Has no effect when the `TauAxisConfig` is assigned to [`TauXYConfig.secondary_x_axis`](xy_config.md#secondary_x_axis). Useful for bar charts where bars originate from zero. Set to `false` when the data range does not contain zero and showing it would waste visible space.
...
Only used when [`type`](#type) is [`CONTINUOUS`](#type) and [`scale`](#scale) is [`LINEAR`](#scale). Has no effect on [`LOGARITHMIC`](#scale) scales, where tick positions follow decade boundaries. The actual tick count may differ from this preference.
...
[`REDUCE_COUNT`](#overlapstrategy) is not valid for [`CATEGORICAL`](#type) axes and falls back to [`SKIP_LABELS`](#overlapstrategy) on them.
...
Only used when [`type`](#type) is [`CONTINUOUS`](#type). When enabled, [`include_zero_in_domain`](#include_zero_in_domain) and [`domain_padding_mode`](#domain_padding_mode) have no effect.
...
Only used when [`range_override_enabled`](#range_override_enabled) is `true`.
...
Only used when [`type`](#type) is [`CONTINUOUS`](#type) and [`range_override_enabled`](#range_override_enabled) is `false`. Has no effect when the `TauAxisConfig` is assigned to [`TauXYConfig.secondary_x_axis`](xy_config.md#secondary_x_axis), because the secondary axis domain is derived from the primary through [`TauXYConfig.secondary_x_axis_transform`](xy_config.md#secondary_x_axis_transform).
