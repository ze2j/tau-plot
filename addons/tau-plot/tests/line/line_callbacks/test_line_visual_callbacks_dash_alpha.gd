...
func make_shared_x_continuous_plot(p_plot: TauPlot, p_title: String, p_interpolation: TauLineConfig.InterpolationMode) -> void:
	...
	visual_callbacks.alpha_callback = Callable(func(series_index: int, sample_index: int, x_value: Variant, y_value: float) -> float:
		const k := 1.6
		var  y := clampf(y_value, 0.0, 1.0)
		return (k + 1.0) * y / (1.0 + k * y)
	)
