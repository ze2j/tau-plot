...
func _make_dataset(p_y_min: float, p_y_max: float) -> TauPlot.Dataset:
	var x := PackedFloat64Array()
	var y := PackedFloat64Array()
	x.resize(SAMPLE_COUNT)
	y.resize(SAMPLE_COUNT)

	var log_min := log(p_y_min) / log(10.0)
	var log_max := log(p_y_max) / log(10.0)
	for i in range(SAMPLE_COUNT):
		var t := float(i) / float(SAMPLE_COUNT - 1)
		x[i] = float(i)
		y[i] = pow(10.0, log_min + t * (log_max - log_min))
...
func _make_plot(p_plot: TauPlot, p_title: String, p_y_min: float, p_y_max: float) -> void:
	var dataset := _make_dataset(p_y_min, p_y_max)
	...
	y_axis.min_override = p_y_min
	y_axis.max_override = p_y_max
...
func _setup_test_4() -> void:
	_make_plot(%TestPlot4, "[0.0002, 0.0005] => no major tick, ticks 2e-4 to 5e-4 all minor, dashed blue lines on 3e-4 and 4e-4, wide labels must fit in the left margin", 0.0002, 0.0005)
